          ...
          >
            ...
            >
              <li class="d-flex align-items-center">
                <router-link to="/institution" class="nav-link d-flex w-100" >
                <b-icon icon="tv" class="text-primary mx-2"></b-icon>
                <span v-if="sideBar">الصفحة الرئيسية</span> </router-link>
              </li>
              <li class="d-flex align-items-center">
                <router-link to="/institution/reports" class="nav-link d-flex w-100">
                  <b-icon icon="bullseye" class="text-primary mx-2"></b-icon>
                  <span v-if="sideBar">الإبلاغات</span></router-link>
              </li>
              <li class="d-flex btn align-items-center border bg-dark shadow-sm rounded" :class="(sideBar)?'mt-5':''">
                <span class="nav-link d-block" @click="logout()">
...
export default {
  ...
  methods: {
    ...
    logout(){
      ...
      this.$router.push("/institution/login");
    }
  },
};
</script>
...
main:before {
  ...
  width: 100%;
  height: 250px;
  background: linear-gradient(45deg, #673ab7 0, #3f51b5 100%) !important;
  position: absolute;
  top: 0;
  left: 0;
  right: 0;
